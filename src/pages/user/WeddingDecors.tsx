...
export const WeddingDecors: React.FC = () => {
  ...
  useEffect(() => {
    const fetchDecors = async () => {
      setIsLoading(true);
      try {
        const res = await fetch(`${API_BASE_URL}/admin/wedding/decors`);
        if (res.ok) {
  ...
  return (
    <PageWrapper className="relative pb-24 pt-4">
      <FloatingOrb className="top-10 -left-20" color="pink" size={400} />
      <FloatingOrb className="bottom-10 right-0" color="violet" size={300} delay={1.4} />
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-4 space-y-8">
        ...
        <div className="space-y-4">
          ...
          <header className="space-y-2">
            ...
            <p className="text-sm text-[var(--text-secondary)] max-w-2xl">
              Select premium mandaps, entryways, canopy setups, and custom lighting designs to define the style of your wedding day.
            </p>
          </header>
        </div>
        ...
        {isLoading ? (
          <div className="py-20 text-center space-y-4">
            ...
            <p className="text-xs text-[var(--text-muted)] font-medium">Fetching decor themes...</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {decors.length === 0 ? (
              <div className="col-span-full text-center py-16 text-xs text-[var(--text-secondary)] border border-white/5 bg-white/5 rounded-2xl">
                No decor themes currently registered.
              </div>
            ) : (
              decors.map((item: any) => (
                <GlassCard key={item.id} className="p-4 border-white/5 space-y-4 flex flex-col justify-between">
                  ...
                  <div className="pt-3 border-t border-white/5 flex justify-between items-baseline text-xs font-bold">
                    <span className="text-[var(--text-muted)] flex items-center gap-1">
                      <Sparkles className="w-3.5 h-3.5 text-[var(--violet-bright)]" /> Theme Decor Setup
                    </span>
